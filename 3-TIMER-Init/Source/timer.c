...
extern char led_status2;
extern char led_status3;
/*********************************************************************************************
* 名称：timer3_init
* 功能：初始化TIM3
...
void timer4_init(unsigned int period, unsigned short prescaler)//TIM_Period为16位的数
{
	TIM_TimeBaseInitTypeDef  TIM_TimeBaseStructure;         //定时器配置
  	NVIC_InitTypeDef  NVIC_InitStructure;                   //中断配置
	 
  	RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM4, ENABLE);  
  	NVIC_InitStructure.NVIC_IRQChannel = TIM4_IRQn;         //TIM3中断通道
  	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;//抢占优先级0
  	NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;      //子优先级1
  	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;         //使能中断
  	...
  	TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1; //时钟分割
  	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;//向上计数模式
  	TIM_TimeBaseInit(TIM4, &TIM_TimeBaseStructure);         //按上述配置初始化TIM3
...
void TIM4_IRQHandler(void)
{
  if (TIM_GetITStatus(TIM4, TIM_IT_Update ) != RESET) {         //如果中断标志被设置
    	TIM_ClearITPendingBit(TIM4, TIM_IT_Update);             //清除中断标志
          led_status3 = ~led_status3;                             //LED灯状态标志位翻转
  	}
